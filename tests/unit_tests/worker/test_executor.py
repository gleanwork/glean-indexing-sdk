...
class TestConnectorExecutor:
    ...
    def create_executor(self, tmp_path: Path) -> ConnectorExecutor:
        ...
        executor._notifications = notifications  # type: ignore[attr-defined]
        return executor
    ...
    def test_log_emits_notification(self, tmp_path: Path):
        ...
        assert len(executor._notifications) == 1  # type: ignore[attr-defined]
        notification = executor._notifications[0]  # type: ignore[attr-defined]
    ...
    async def test_run_fetch_phase(self, tmp_path: Path):
        ...
        methods = [n.method for n in executor._notifications]  # type: ignore[attr-defined]
    ...
    async def test_run_fetch_phase_abort(self, tmp_path: Path):
        ...
        record_fetched_count = sum(
            1
            for n in executor._notifications  # type: ignore[attr-defined]
            if n.method == "record_fetched"
        )
...
"""
        ...
        complete_notifications = [
            n
            for n in executor._notifications  # type: ignore[attr-defined]
            if n.method == "execution_complete"
        ]
        assert len(complete_notifications) == 1
        assert complete_notifications[0].params is not None
